...
//
// Replace old GroupNoiseModel/IncoherentAddNoise with UncorrelatedAddNoise.
// Assumes noise model stores (WCT base units):
//   - freq_ghz : GHz (== 1/ns in WCT base time units)
//   - avg_mag  : MV  (WCT base voltage)
//
// And UncorrelatedAddNoise adds noise in MV to frames in MV.
...
// -----------------------------------------------------------------------------
// HARD-CODE uncorrelated noise model path (MV + GHz model)
// -----------------------------------------------------------------------------
local uncorr_model = "/exp/dune/app/users/avighosh/noise_model/model_files/uncorrelated_noise_model_WCT_MV_GHz.json.bz2";
...
// -----------------------------------------------------------------------------
// Uncorrelated noise injection branch
// -----------------------------------------------------------------------------
...
// one uncorrelated noise node per lane
local uncorrnoises = [
  g.pnode({
